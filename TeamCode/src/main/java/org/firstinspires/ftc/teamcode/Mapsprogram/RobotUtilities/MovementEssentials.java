package org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities;
...
import org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.PiecewiseFunction;
...
import org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.CurvePoint;

import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MovementEssentials.profileStates.gunningIt;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MovementVars.movement_turn;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MovementVars.movement_x;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MovementVars.movement_y;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MyPosition.AngleWrap;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MyPosition.subtractAngles;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MyPosition.worldAngle_rad;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MyPosition.worldXPosition;
import static org.firstinspires.ftc.teamcode.Mapsprogram.RobotUtilities.MyPosition.worldYPosition;
...
public class MovementEssentials {
    ...
    public static void goToPosition(double targetX, double targetY, double point_angle, double movement_speed, double point_speed) {
        ...
        if(state_movement_y_prof == profileStates.gunningIt) {
            if(relative_abs_y < Math.abs(SpeedOdmeter.currSlipDistanceY() * 2) || relative_abs_y < 3){
                state_movement_y_prof = state_movement_y_prof.next();
            }
        }
        if(state_movement_y_prof == profileStates.slipping){
            movement_y_power = 0;
            if(Math.abs(SpeedOdmeter.getSpeedY()) <  0.03){
                state_movement_y_prof = state_movement_y_prof.next();
            }
        }
        ...
        if(state_movement_x_prof == profileStates.gunningIt) {
            if(relative_abs_x < Math.abs(SpeedOdmeter.currSlipDistanceY() * 1.2) || relative_abs_x < 3){
        ...
        if(state_turning_prof == profileStates.gunningIt) {
            ...
            if(Math.abs(rad_to_target) < Math.abs(SpeedOdmeter.currSlipAngle() * 1.2) || Math.abs(rad_to_target) < Math.toRadians(3.0)){
                state_turning_prof = state_turning_prof.next();
            }

        }
        if(state_turning_prof == profileStates.slipping){
            if(Math.abs(SpeedOdmeter.getDegPerSecond()) < 60){
                state_turning_prof = state_turning_prof.next();
            }
    ...
    public static movementResult gunToPosition(double targetX, double targetY,double point_angle,
        ...
        double currSlipY = (SpeedOdmeter.currSlipDistanceY() * Math.sin(worldAngle_rad)) +
                (SpeedOdmeter.currSlipDistanceX() * Math.cos(worldAngle_rad));
        double currSlipX = (SpeedOdmeter.currSlipDistanceY() * Math.cos(worldAngle_rad)) +
                (SpeedOdmeter.currSlipDistanceX() * Math.sin(worldAngle_rad));
    ...
    public static movementResult pointAngle(double point_angle, double point_speed, double decelerationRadians) {
        ...
        double velocityAdjustedRelativePointAngle = AngleWrap(relativePointAngle-SpeedOdmeter.currSlipAngle());

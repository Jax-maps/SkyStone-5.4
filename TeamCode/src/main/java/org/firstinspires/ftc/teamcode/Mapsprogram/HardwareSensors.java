...
public class HardwareSensors
{
    ...
    public enum DEACQUIRE_ACTIVITY {
        IDLE,
        DEACQUIRE,
        DOWN
    }
    ...
    public DEACQUIRE_ACTIVITY deacquireState = DEACQUIRE_ACTIVITY.IDLE;

    //Runtimes for arm movements
    public ElapsedTime acquireTime = new ElapsedTime();
    public ElapsedTime deacquireTime = new ElapsedTime();

    // Variables
    private final int DENEST = 1500; // Subject to change
    private final int LIFT_TO_DENEST = 2850; // Subject to change
    ...
    public void performDenesting() {
        switch(denestState) {
            case LIFTING_TO_ROTATE:
                ...
                if(acq2.getCurrentPosition() > 2700) {
    ...
    public boolean startAcquiring(){
        ...
        if(deacquireState == DEACQUIRE_ACTIVITY.IDLE) {
    ...
    public boolean startDeacquire(){
        // This is more of a design pattern to show if the start happened or not.
        boolean startingDeacquire;

        // We don't want to start acquiring if it is in the middle of acquiring.
        if(acquireState == ACQUIRE_ACTIVITY.IDLE) {
            if (deacquireState == DEACQUIRE_ACTIVITY.IDLE) {
                startingDeacquire = true;
                fineMovemnt.setPosition(0);

                // This activates the perform starting to  function to start executing.
                deacquireState = DEACQUIRE_ACTIVITY.DEACQUIRE;
            } else {
                startingDeacquire = false;
            }
            return startingDeacquire;
        }
        return false;
    }
    public void performDeacquire(){
        switch(deacquireState) {
            case DEACQUIRE:
                // When finemovement reaches this position, that means it finished lifting.
                if(fineMovemnt.getPosition() == 0) {
                    // Time to start securing the block
                    ...
                    // Set our state to the next step.  Next time through the OpMode loop
                    // it will get to the next step.
                    deacquireState = DEACQUIRE_ACTIVITY.DOWN;
                }
                break;
            case DOWN:
                // When arm reaches this position, that means it finished rotating.
                if(arm.getPosition() == 0.6){
                    // Time to start going up
                    fineMovemnt.setPosition(0.75);
                    // Set our state to the next step.  Next time through the OpMode loop
                    // it will get to the next step.
                    deacquireState = DEACQUIRE_ACTIVITY.IDLE;
                }
                break;
            case IDLE:
                // Do nothing.
